...
if %ChoixMenu% NEQ 0 (
	...
	if !upupon! == 1 (
		...
		start "Start WU" /D "%~dp0Routines\" /wait /HIGH StartWindowsUpdateServices.bat
	...
	if !byebyeups! == 1 (
		...
		if %one% == 1 (
			...
			start "RidOFGWX" /D "%~dp0Routines\" /wait /HIGH Riddof_GWX_FileNReg.bat
		...
		if %three% == 1 (
			...
			start "NoTelem" /D "%~dp0Routines\" /wait /HIGH Config_NoTelemetry.bat
		)
	...
	:: test pour passage 8.1 vers 10
			...
			start "NoUpgrade" /D "%~dp0Routines\" /wait /HIGH NoUpgrade.bat
		)
	)
	echo:
	...
	if %four%==1 (
			...
			start "Conf WU policies" /D "%~dp0Routines\" /wait /HIGH ConfigUpdatesPolicies.bat
			)

	if %five%==1 (
			echo:
			echo lancement de %~dp0Routines\TweakOS_Light.bat
			start "Tweaks OS - Light version" /D "%~dp0Routines\" /wait /HIGH TweakOS_Light.bat
			)

	if %six%==1 (
			echo:
			echo lancement de %~dp0Routines\CleaningJob.bat
			start "Cleaning" /D "%~dp0Routines\" /wait /HIGH CleaningJob.bat
			)
	...
	if %seven%==1 (
		...
		start "Disable WU" /D "%~dp0Routines\" /wait /HIGH Enable-DisableWinUpdate.bat d
		)	
)
...
if %ChxAdvMenu% NEQ 0 (
	...
	if %A1%==1 (
		echo:
		echo lancement de %~dp0Routines\ConfigUpdatesPolicies.bat
		start "Conf WU en Auto" /D "%~dp0Routines\" /wait /HIGH ConfigUpdatesPolicies.bat
